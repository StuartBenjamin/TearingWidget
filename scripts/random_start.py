...
def run_dcon_on_equilibria(out_path,dcon_executable_dir,rdcon_executable_dir,n=0,cases_in_batch=0,
                           **kwargs):
    ...
    for i in range(1,total_batches+1):
        ...
        if os.path.isdir(eqdsk_dir_name):
    ...
    for i in read_write_dir_vec:
        ...
        if len(equil_names)>0:
            for j in equil_names:
                if not os.path.isfile(i[1]+"/"+j+"_dcon_xr"): # and os.path.isfile(i+"/"+j):
                    ...
                    dcon_ran,rdcon_ran,MRE_xr=run_DCON_on_equilibrium2(i[0]+"/"+j,
                            working_dir=working_dir,dcon_executable_dir=dcon_executable_dir,
                            rdcon_executable_dir=rdcon_executable_dir,
                            **kwargs)
                    if dcon_ran or rdcon_ran:
...
def _gen_n_successful_cases(n,out_path="dset",return_total=True,save=True,save_equil=True,run_tokamaker=False,cases_in_batch=100,inputpath="/Users/sbenjamin/Desktop/TEARING_WIDGET/scripts",radas_dir_path='/Users/sbenjamin/Desktop/TEARING_WIDGET/cfspopcon/radas_dir',
    ...
    if not os.path.isdir(out_path):
        os.mkdir(out_path)
    if not os.path.isdir(out_path+"/OFT_working_dir"):
        os.mkdir(out_path+"/OFT_working_dir")
    os.chdir(out_path+"/OFT_working_dir")
    mygs = TokaMaker()
    if not os.path.isdir(out_path+"/dcon_working_dir"):
        os.mkdir(out_path+"/dcon_working_dir")
            ...
            for i in range(a.shape[0]):
                ...
                if run_tokamaker and MC_case_datasetout[0]: 
                    ...
                    if run_phys_suite:
                        ...
                        ideal_MHD_stable3=False
                        #if flag1==0: 
                            #os.chdir(out_path+"/dcon_working_dir")
                        ...
                        if flag3==0:
                            try:
                                os.chdir(out_path+"/dcon_working_dir")
                                dcon_ran3,rdcon_ran3,MRE_xr3=run_DCON_on_equilibrium2(g_file3_name,newq0=q0new3,working_dir=working_dir_internal,**run_DCON_on_equilibrium_args_dict) #focus is on getting MRE data for this one...
                            except:
...
def save_gfile(mygs, MC_ind,out_path,batch_num,total_batches,gname='g_MCi',verbose=False,Jfrac=0.0,nr=700,nz=700,maxsteps=1000000,lcfs_pad=1e-6,ttol=1e-9,run_simple=True):
    ...
    if run_simple: #avoids a mystery seg fault at the expense of throwing an error if a single case fails
        if Jfrac==0.0:
            gname_tot = out_path+f"/eqdsks_b{batch_num}of{total_batches}/"+gname+f"{MC_ind}"
            ...
            gname_tot = out_path+f"/eqdsks_b{batch_num}of{total_batches}/"+gname+f"{MC_ind}_Jfrac{Jfrac}"
            gname_tot = gname_tot.replace("//", "/")
            TokaMaker.save_eqdsk(mygs,gname_tot
                            ,nr=nr,nz=nz,lcfs_pad=lcfs_pad,maxsteps=maxsteps,ttol=ttol)
        return gname_tot
